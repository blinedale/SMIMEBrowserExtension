...
def ocsp_request(signature):
    ...
    client_certificate_array = find_certificate_with_subject_line_pattern(cli_output,
                                                                          client_certificate_subject_pattern)
    ...
    cli_output = exec_and_return_byte(
        'openssl ocsp -issuer %s -cert %s -url %s' % (issuer_certificate_path, client_certificate_path, ocsp_url))
    ocsp_result = parse_ocsp_response(cli_output, client_certificate_path)
    print('ocsp_result : %s' % ocsp_result)

    # Cleaning up files
    os.remove(client_certificate_path)
    os.remove(issuer_certificate_path)
    os.remove(signature_decoded_path)
    os.remove(signature_raw_path)
...
def exec_and_return_byte(command_string):
    result = subprocess.run(command_string, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
