...
export function getSavedResult(db, mailId) {
  return new Promise(resolve => {
    ...
    const transaction = db.transaction([dbConfig.stores.results], "readonly");
...
export function saveResult(db, resultObject) {
  return new Promise(resolve => {
    ...
    const transaction = db.transaction([dbConfig.stores.results], "readwrite");
    const resultStore = transaction.objectStore(dbConfig.stores.results);
    ...
    request.onsuccess = function(event) {
      console.log(`Saved result for mail id ${resultObject.mailId}`);
      console.log(request.result);
      console.log(event);
      return resolve(request.result);
    }
